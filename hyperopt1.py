...
def random_search(problem, function_evaluations=180, **kwargs):
    """
...
def good_bad(gamma, hyper_parameters):
    ...
    if 23 in sorted_df.columns:
        sorted_df = sorted_df.drop(columns=[23]) 
...
def categorical_pdf(x, n, ranges):
    #n = len(samples)
    c = Counter(x)
    return c
...
def tpe(problem, function_evaluations=150, random_warmup=30, gamma=0.2, n=24,**kwargs):
    """
    ...
    for k in range(function_evaluations):
        ...
        for i in good_df.columns.difference(['loss']):
            ...
            if RANGES[i]['type'] == 0:
                ...
                lx = categorical_pdf(x, n,RANGES[i])
                gx = categorical_pdf(y, n, RANGES[i])

                samples = np.random.choice(RANGES[i]['range'],n)

                densities_lx = np.zeros(n)
                densities_gx = np.zeros(n)
                for i in range(n):
                    densities_lx[i] = lx[samples[i]]
                    densities_gx[i] = gx[samples[i]]

                EI = calculate_EI(densities_lx, densities_gx)
                EI_max = np.argmax(EI)
                value = samples[EI_max]
                x_star[i] = value
